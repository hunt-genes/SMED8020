## Modified script from https://github.com/ilarsf/gwasTools
...
option_list <- list(
  ...
  make_option("--maintitle", type="character", default="",
    help="Plot title"),
  make_option("--thinratio", type="numeric", default="0.05",
    help="Thinning ratio to avoid plotting low -log pvalues")  
)
...
prethin2 <- prethin2[order(rnorm(length(prethin2)))[1:(length(prethin2)*opt$thinratio)]]
...
plotdata <- data.frame(
    ...
    'pch'=20,
    'highlightColor'=NA,
    'pcol'=chrColors[as.character(CHR)],
    check.names=F)
endPos <- 0
plotPos <- numeric(0)
chrLab <- numeric(0)
...
png(filename = paste0(opt$prefix,"_Manhattan.png"), width = opt$width, height = opt$height, pointsize = opt$pointsize)
	...
	if (length(chrLab)>1) {
          ...
          axis(1,at=chrLab[seq(1,length(chrLab))],
            labels=chrs[1:length(chrLab)][seq(1,length(chrLab))],
            las=1,tick=F,cex.axis=1.5,cex.lab=1.5,line=2)
	}
