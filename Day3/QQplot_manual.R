...
#### Things to edit
#--input LDL_METAL_META1.tbl --pvalue P-value --af Freq1 --prefix LDL_METAL_MultiStudy --break.top 120

input <- "LDL_METAL_META1.tbl"
pvalue <- "P-value"
af <- "Freq1"
prefix <- "LDL_METAL_MultiStudy"
break.top <- 120
#### Things to edit
...
if (grepl('.gz',input)) {
  gwas <- fread(paste(sep=" ","zcat",input),header=T)
} else {
  gwas <- fread(input, header=T)
}

#convert pvalues to -log10pvalue or use existing values in that scale
if(!log10p) {
  gwas[[pvalue]]<- as.numeric(gwas[[pvalue]]) #handle any NAs
...
if (maf %in% colnames(gwas)) { 
  ...
  minMAF<-min(gwas[[maf]]) #minMAF of all input data
} else if (af %in% colnames(gwas)) {
  gwas$maf<-as.numeric(gwas[[af]])
...
if (minMAF > 0) { #minMAF provided so filter by MAF
  if (minMAC > 0 ) {
    stop("Please only provide either --minMAF or --minMAC but not both\n")
  } else {
    gwas <- gwas[gwas[[mafcol]] > minMAF] #filter by MAF
    minMAF<-min(gwas[[mafcol]]) #new minMAF
    print(summary(gwas[[mafcol]])) #print MAF 
  }
} else if (minMAC > 0) { #minMAC provided so filter by MAC
  if (mac %in% colnames(gwas)) {
    gwas<-gwas[gwas[[mac]] > minMAC] #filter by MAC
    print(summary(gwas[[mac]])) #print MAC
  } else if (ac %in% colnames(gwas)) {
    gwas$mac<-gwas[[ac]]
    gwas$mac[which(gwas$mac > gwas[[sample.size]])] <- 2*gwas[[sample.size]][which(gwas$mac > gwas[[sample.size]])] - gwas$mac[which(gwas$mac > gwas[[sample.size]])] #convert to MAC
    gwas<-gwas[mac > minMAC] #filter by MAC
    print(summary(gwas$mac)) #print MAC
  } else {
...
lambda_file_name<-paste0(prefix,"_lambda.txt")
lambda_df<-NULL
...
if (pdf==TRUE) { #plot as pdf, default for height/width/point size are customized for png
  pdf(filename = paste0(prefix,"_QQ.pdf"), width = width, height = height, pointsize = pointsize)
} else {
  png(filename = paste0(prefix,"_QQ.png"), width = width, height = height, pointsize = pointsize)
}
