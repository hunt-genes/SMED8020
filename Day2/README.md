...
Note: The Plink.exe file needs to have the same file path as the data
...
Store them on your local computer and direct the terminal to them by writing ```cd filepath```
A suggested structure would be a folder that has Plink, the three data day2 files and a subfolder called output.
